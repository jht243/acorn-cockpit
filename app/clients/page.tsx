          ...
          <div className="grid grid-cols-12 gap-5">
            <div className="col-span-12 card card-pad flex flex-wrap items-center gap-3">
              ...
              <div className="text-xs ml-auto" style={{ color: "var(--ink-soft)" }}>
                Showing <span className="font-semibold text-[var(--ink)]">{rows.length}</span> of {clients.length} · {fmtUSD(totalNW)} net worth
              </div>
            </div>
            ...
            <div className="col-span-12 card">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.map((c) => {
                    const nw = netWorth(c);
                    const open = c.actions.filter((a) => a.status !== "done").length;
                    return (
                      <tr key={c.id} className="row-hover border-t" style={{ borderColor: "var(--line)" }}>
                        ...
                        <td className="px-5 py-3" style={{ color: "var(--ink-soft)" }}>
                          <div className="text-xs">{c.lastContact}</div>
                          {c.lastContactSignal && (
                            <div className="text-[10px] mt-0.5 flex items-center gap-1">
                              <SourceBadge source={c.lastContactSignal.source} />
                            </div>
                          )}
                        </td>
